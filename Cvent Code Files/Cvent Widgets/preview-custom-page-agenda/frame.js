...
async function load() {
  ...
  const [mod, dump] = await Promise.all([
    ...
    fetch(`./data/page-agenda-dump.json${bust}`).then((r) => r.json()),
  ]);
  Widget = mod.default;
  if (!customElements.get("preview-page-agenda-widget")) customElements.define("preview-page-agenda-widget", Widget);
