...
const WIDGET_DIR = "../custom-page-agenda";
const STORE_KEY = "page-agenda-preview-config";
...
async function main() {
  ...
  if (!customElements.get("preview-page-agenda-editor")) customElements.define("preview-page-agenda-editor", Editor);
  ...
  const applyPreset = (key) => {
    ...
    api.update(currentConfig);
    try { localStorage.setItem("page-agenda-preview-bp", key); } catch {}
  };
  let bp = "desktop";
  try { bp = localStorage.getItem("page-agenda-preview-bp") || "desktop"; } catch {}
  $("width").value = bp;
  applyPreset(bp);
