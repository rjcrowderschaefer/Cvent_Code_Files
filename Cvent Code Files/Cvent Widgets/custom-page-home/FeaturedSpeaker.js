...
export class FeaturedSpeaker extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Optional small label above the name (e.g. "Moderator"), from the host
    // widget: cfg.cardEyebrow(speaker) -> string.
    let cardEb = null;
    const ebText = typeof cfg.cardEyebrow === "function" ? String(cfg.cardEyebrow(sp) || "") : "";
    if (ebText || cfg.cardEyebrowReserve) {
      // Reserve: an empty, invisible label keeps names aligned across a grid
      // row when only some cards carry one.
      cardEb = document.createElement("p");
      cardEb.className = "cardEb";
      cardEb.textContent = ebText || "\u00a0";
      if (!ebText) { cardEb.style.visibility = "hidden"; cardEb.setAttribute("aria-hidden", "true"); }
    }
  ...
  openModal() {
    ...
    // "Session" for one, "Sessions" for several.
    const many = cfg.sessionsHeaderText || "Sessions";
    const SINGULAR = { Sessions: "Session", Sesiones: "Sesión", "Sessões": "Sessão", SESSIONS: "SESSION" };
    m.sessionsHdr.textContent = appearsIn.length === 1 ? (cfg.sessionHeaderText || SINGULAR[many] || many) : many;
  ...
  _eyebrowLabel(sp) {
    ...
    if (/moderator|moderadora?/i.test(hay)) return cfg.moderatorLabel || "Moderator";
