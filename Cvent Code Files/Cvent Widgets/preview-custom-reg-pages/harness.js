...
const STORE_KEY = "reg-pages-preview-config";
...
async function main() {
  ...
  if (!customElements.get("preview-reg-pages-editor")) customElements.define("preview-reg-pages-editor", Editor);
  ...
  const applyPreset = (key) => {
    ...
    api.update(currentConfig);
    try { localStorage.setItem("reg-pages-preview-bp", key); } catch {}
  };
  let bp = "desktop";
  try { bp = localStorage.getItem("reg-pages-preview-bp") || "desktop"; } catch {}
  $("width").value = bp;
  applyPreset(bp);
