...
async function load() {
  ...
  const [mod, dump] = await Promise.all([
    ...
    fetch(`./data/page-speakers-dump.json${bust}`).then((r) => r.json()),
  ]);
  Widget = mod.default;
  if (!customElements.get("preview-page-speakers-widget")) customElements.define("preview-page-speakers-widget", Widget);
