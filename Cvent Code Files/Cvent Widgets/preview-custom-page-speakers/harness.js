...
const WIDGET_DIR = "../custom-page-speakers";
const STORE_KEY = "page-speakers-preview-config";
...
async function main() {
  ...
  await fetch(`${WIDGET_DIR}/editor-kit.js`, { cache: "reload" }); // fixed-specifier import
  ...
  if (!customElements.get("preview-page-speakers-editor")) customElements.define("preview-page-speakers-editor", Editor);
  ...
  const applyPreset = (key) => {
    ...
    api.update(currentConfig);
    try { localStorage.setItem("page-speakers-preview-bp", key); } catch {}
  };
  let bp = "desktop";
  try { bp = localStorage.getItem("page-speakers-preview-bp") || "desktop"; } catch {}
  $("width").value = bp;
  applyPreset(bp);
