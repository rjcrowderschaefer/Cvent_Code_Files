...
async function load() {
  ...
  const [mod, dump] = await Promise.all([
    ...
    fetch(`./data/page-contact-dump.json${bust}`).then((r) => r.json()),
  ]);
  Widget = mod.default;
  if (!customElements.get("preview-page-contact-widget")) customElements.define("preview-page-contact-widget", Widget);
