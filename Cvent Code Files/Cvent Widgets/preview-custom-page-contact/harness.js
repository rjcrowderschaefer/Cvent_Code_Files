...
const WIDGET_DIR = "../custom-page-contact";
const STORE_KEY = "page-contact-preview-config";
...
async function main() {
  ...
  if (!customElements.get("preview-page-contact-editor")) customElements.define("preview-page-contact-editor", Editor);
  ...
  const applyPreset = (key) => {
    ...
    api.update(currentConfig);
    try { localStorage.setItem("page-contact-preview-bp", key); } catch {}
  };
  let bp = "desktop";
  try { bp = localStorage.getItem("page-contact-preview-bp") || "desktop"; } catch {}
  $("width").value = bp;
  applyPreset(bp);
