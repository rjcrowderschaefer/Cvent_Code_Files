  ...
  connectedCallback() {
    ...
    style.textContent = `
      ...
      .card {
        ...
        overflow: hidden;
        width: 100%;
        max-width: calc(84px + 800px); /* content capped at 800 */
      }
      ...
      .avatar { width: 50px; height: 50px; border-radius: 50%; border: 1px solid #FFFFFF;
      ...
      .modalHeader {
        ...
        padding: 12px 16px;
        background: ${modalHeaderBg};              /* NEW */
        border-bottom: 1px solid ${modalDivider};  /* NEW */
      }
    ...
    this.applyThemeStyle(timeStartEl, this.theme.paragraph);
    this.applyTypographyOverrides(timeStartEl, this.config.typography?.sessionTime, true);

    const timeEndEl = document.createElement('div');
    timeEndEl.classList.add('timePart', 'timeEnd');
    timeEndEl.textContent = endText || '';
    this.applyThemeStyle(timeEndEl, this.theme.paragraph);
    this.applyTypographyOverrides(timeEndEl, this.config.typography?.sessionTime, true);
    ...
    this.applyThemeStyle(titleEl, this.theme.header2, { margin: '0' });
    this.applyTypographyOverrides(titleEl, this.config.typography?.sessionName, true);
    content.append(titleEl);

    // Description
    if (this.config.showDescription && s.description) {
      const desc = document.createElement('div');
      desc.innerHTML = s.description;
      this.applyThemeStyle(desc, this.theme.mainText);
      this.applyTypographyOverrides(desc, this.config.typography?.sessionDescription, true);
      content.append(desc);
    }
  ...
  openModalForSpeaker(sp) {
    ...
    if (appearsIn.length) {
      appearsIn.forEach(sess => {
        ...
        this.applyTypographyOverrides(nameSpan, this.config.typography?.modalSessionName, true);
        ...
        this.applyTypographyOverrides(dtSpan, this.config.typography?.modalSessionDateTime, true);
